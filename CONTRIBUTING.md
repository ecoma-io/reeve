...
| `ECOMA_REEVE_MODEL`  | this repo | The single model every dogfood duty runs on         |

Dogfood is a ONE-MODEL configuration: `ECOMA_REEVE_MODEL` is one id, used for
every duty, every chain, and every draft the run grades. A judge panel or a
screen roster is deliberately not wired — the `judge-models` and `screen-models`
...
```
ECOMA_REEVE_MODEL  vendor/big=House model
```

Name the id here for a reason beyond taste: the id is masked out of the log,
and the run report is not the log — it is a page the job writes for itself,
where an unnamed id would appear as itself. The name is also what a reader of
that page wants anyway.
...
version that does not leave a stale copy behind. A model id belongs to the thing
making the call: dogfood wants the one model it has measured its scoring
against, and a consumer who wants a judge panel or a screen roster adds
...
[the roadmap](docs/doctrine/north-star.md#7-roadmap) is done.** Until then every release
