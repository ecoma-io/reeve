 ...
 * wrong: a group that needed exactly `max-requests` requests and had nothing
 * left to ask for reads identically, at the end, to one that had a locale
 * turned away — the meter sits at the ceiling either way. `denied` is set
 * exactly once, at the moment `budgetExhausted` itself answers `true` — every
 * call site below only asks the question immediately before a real piece of
 * work it would otherwise do, so that answer is always a genuine denial.
 */
 ...
 * classification, drafting and judging combined, the same total the summary's
 * own spend table adds up to. `null` is no bound, the default, and never
 * exhausted.
 *
 ...
 *
 * Checked at each place a fresh request is about to be spent — before each
 * document group, before each stale locale — rather than once up front,
 * because the budget can run out partway through any of those, and what
 * already happened stands: a draft already written and published is not
 * un-published because the next locale could not be started.
 */
