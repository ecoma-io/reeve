 ...
 *
 * The ordinary-rendering cases and the adversarial ones live in one file on
 * purpose. They were two, and both opened with the same thirty-one-line `run()`
 * builder enumerating all twenty-six fields of `Run` — so every field added to
 * `Run` was two identical edits, and the day one of them was missed the two
 * builders would have drifted into testing two different shapes.
 */
