...
export function extractEdges(
  ...
  // lines (until the closer that is not on the opener's line closes it).
  ...
  for (const [line, text] of file.lines) {
    ...
    if (trimmed.startsWith("/*")) {
      // A line whose trimmed form opens a block comment that does not close
      // on the same line leaves the rest of the comment open; one that closes
      // on the same line falls through so code after the `*/` still extracts.
