...
| `review`      | Reviews a pull request — deterministic pre-checks plus risk-tiered model passes (one on low-risk diffs, more on high-risk ones), synthesized (deduplicated, ranked, contradiction-aware) into a single owned comment that tracks its findings across `synchronize` events instead of reposting them. Top rung: granted nothing until a warrant names it. | [Reference](docs/reference/duties/review.md)      |
| `remediation` | Turns a review's standing findings into deterministic remediation proposals — recorded on the job summary, never written to the repository. Proposal-only: an over-grant of `edit-file` or `open-pr` fails red.                                                                                                                                          | [Reference](docs/reference/duties/remediation.md) |
...
deterministic pre-checks first — ignore, generated, blocked — and a model pass
after them, priced by how risky the diff is: one pass on a low-risk diff, an
independent second opinion on medium, an adversarial verification pass on top
for high. Each pass reads the diff independently and the synthesis correlates
the findings — the same finding from two passes is reported once,
corroborated; a disagreement at the same line is annotated, not silently kept
or dropped. The verdict lands once, as one owned summary comment plus an owned
inline thread per finding the diff proves, and tracks its findings across
`synchronize` events instead of reposting. Rules are your own
(`.github/reeve-rules.yml`), risk depth is your own (`.github/reeve-risk.yml`)
and a maintainer always owns the diff. [Review](docs/reference/duties/review.md).
