...
| `max-body-chars` | `6000`               | How much of a long thread reaches the prompt, or `none` for no bound at all. The tail is dropped, and the run logs a warning. `0` is refused — write `none` for that meaning instead. |
