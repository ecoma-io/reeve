...
**Warrant grant:** `label` and `comment` are granted by default —
`DEFAULT_CAPABILITIES` for this duty is `[label, comment]`. `close` needs
naming in `.github/reeve.yml`'s `duties.lifecycle` before this duty ever
closes a thread. The `duties:` block is the whole authority — nothing on
the workflow can widen it. See
...
| -------------- | -------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `github-token` | no       | `${{ github.token }}` | Token used to read a thread's comments and events, and to apply what the warrant grants.                                                                                                                                                                               |
...
| `skipped`         | Outside `sweep`: `true` when this thread had no policy to run against (no `lifecycle:` key, `duties:` not naming this duty, a closed thread, a `threads:` kind mismatch, a draft or other permanent exemption). Under `sweep`: every thread skipped for one of those reasons — including the ones filtered out of the listing before a per-thread read was spent, not only `exempt.labels` matches. |
...
| `due-not-granted` | Outside `sweep`: how many steps were due this run but skipped whole because the warrant withheld a capability the step's action needed. Under `sweep`: the same count, summed across every thread examined.                                                                                                                                                                                         |
...
| ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| No `lifecycle:` key, or `duties:` does not name this duty         | `skipped` populated, nothing applied, **green**                                                                      |
