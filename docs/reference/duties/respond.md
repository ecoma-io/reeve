...
jobs:
  respond:
    ...
    steps:
      ...
      - uses: ecoma-io/reeve/respond@v0.6
        with:
...
| `judge-models`    | no       | _(empty)_                   | A panel that picks the best draft. `\|` adds a seat (one more vote); `,` is a fallback inside one, as in `models` — same grammar as [`translate`'s](translate.md#configuration).                                       |
| `drafts`          | no       | `1`                         | Attempts per run, scored and judged. A first reply is worth more than one attempt. Spread across the roster: draft `n` starts at the `n`th model, so three drafts ask three different models when the roster has them. |
...
| `corrections-dir` | no       | `.reeve/corrections`        | The memory store. Empty is the cold-start case and works.                                                                                                                                                              |
