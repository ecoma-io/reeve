...
| `state-branch`    | no       | `reeve/corrections`         | A branch to write corrections to, instead of the default branch. When set, correction files are committed to this branch and a draft PR is opened for review. `record` and `open-pr` must both be granted. Empty writes directly to the default branch. |
...
corrections are written to that branch and a draft PR is opened;
set `state-branch: ""` to write directly to the default branch instead.
