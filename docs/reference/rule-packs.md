...
| `generated`                     | **Union of every non-empty list.** An empty union falls back to the built-in defaults (minified suffixes, snapshots, lockfiles, and build-output directories — see [the review duty's `generated` section](duties/review.md#deterministic-findings-before-any-model)). With no packs this reproduces today exactly. |
