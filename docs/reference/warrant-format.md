...
| `auto-close`        | no       | Whether PRs for obsolete updates are auto-closed. Default `false`. Only dependa's own marker-bearing PRs on `reeve/dependa/*` branches are ever considered.                   |
| `auto-rebase`       | no       | Whether PRs are auto-rebased when the target branch advances. Default `true`. Rebase force-updates only dependa's own branch; a branch with human commits is left conflicted. |
...
| `propose`   | Open or update a pull request adding or retiring taxonomy labels, from a monorepo's own package layout. `triage`-only, sweep-only. Also the grant `remediation` reads to derive deterministic remediation proposals from a review's findings — proposal-only, no repository write. Needs `contents: write` and `pull-requests: write` on the token for triage's pull-request path. | off                          |
...
`duplicate`, `respond`, `review`, and `remediation` are the exceptions with no
default at all — not even the cheapest one. See [Duties](../guides/warrant.md#duties)
for why.
