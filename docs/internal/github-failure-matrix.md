...
**Ruled and implemented: a versionless payload is a LOUD cold start** — `corrupt` with an accurate reason, not a silent `none`. State was found and discarded, which a reader has to be able to learn, and because the legacy window is closed this branch should essentially never fire: a silent branch that should never execute is a branch nobody learns fired. `corrupt` here means exactly "there was state, it is not usable, cold-start and say so" — the run stays **green**, the memory is rebuilt from the thread exactly as `none` did, and the reason names the missing version rather than claiming a checksum failure (there is no checksum on a versionless payload to have failed). All three are pinned end to end by `main.integration.test.ts`'s `announces a versionless envelope, cold-starts from the thread, and stays green`, which was verified to go red both when the silent variant is restored and when the reason string is made inaccurate. The deciding evidence is that the legacy window is closed — `git merge-base --is-ancestor fec2feb 0fa21e6` confirms v2 shipped **before** the 0.8.0 release, so a versionless envelope can only sit on a thread not reviewed since before 0.8.0, and the first review after upgrade rewrites it. The cost is at most one cold start on a stale thread, and cold start is an already-supported, already-tested state. `migrateV1` and `isV1Findable` are deleted; nothing else called them.
...
**Adjudicated 2026-08-18: fixed, narrowly.** A 403 is capacity only when its `.response.headers` are GitHub naming the limit — a `retry-after` in delta-seconds, or `x-ratelimit-remaining` at exactly 0 (the number, or `"0"` trimmed; never coerced). A bare 403, a 403 on a healthy quota, a 403 whose headers are absent, a string, an array, or reachable only through a prototype, all stay the red permission error they were. No other status gained a header-driven path, and 401 is excluded outright. Pinned in `forge.test.ts` ("isCapacityError — GitHub's 403/429 rate-limit ambiguity") and `forge.adversarial.test.ts` ("isCapacityError — header shapes that must not decide a run's colour").

**Hardened after an independent audit.** The first cut read headers with `Object.entries`, which is `[]` for a WHATWG `Headers` — so a rate-limit 403 from any fetch-based client was still red, the defect silently not fixed. It could also throw from a hostile enumerable getter, and `isCapacityError` runs inside `catch` blocks at twelve sites where a throw replaces the failure being classified. `headerValue` now tries a `Headers`-like `.get`, an iterable of pairs, then own keys, and is total for every input; `retry-after` is bounded to delta-seconds (`"unknown"`, `"-5"`, `"0x0"`, `"1e3"` and an HTTP-date are not proof of a limit — GitHub sends seconds, and `Date.parse` accepts too much to admit dates safely). The audit also found the green side pinned only against `forge`'s own tests: deleting the 403 branch left the other 184 files all passing. Call-site pins now exist in `state-branch.test.ts` (`ensureBranch`, `publishState`) and `diagnose.test.ts` (`classify` reports green, not a token refusal), and both directions are mutation rows in `tools/mutation.mjs`.
