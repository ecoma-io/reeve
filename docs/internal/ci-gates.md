...
| ---: | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------- | ----------------------------------- |
|    1 | `pnpm test:mutation`                                      | For each named regression, **the suite fails**. This is the only metric here that measures protection rather than execution.                                                    | Only the 54 seams the table names. Silent about everything not in it.                               | ~90 s harness, ~3 min job | **mandatory (new this round)**      |
...
- a new `mutation` job (`ci.yml:165-189`) running `pnpm test:mutation`, which now ends `54 KILLED · 0 SURVIVED · 0 STALE`;
...
| Measurement               | Value                                                 |
| ------------------------- | ----------------------------------------------------- |
| Full mutation table       | 54 mutations, **~90 s** wall clock, four cores        |
| `--stage fast` subset     | 42 mutations, **~65 s**                               |
| `pnpm test` with coverage | 4854 tests over 186 files, **~250 s** on the same box |

The whole mutation table costs a little over a third of what the test suite already costs, and it runs in parallel with it, so the pipeline's wall clock does not move at all. Staging it would save about twenty-five seconds — the fast subset is not proportionally cheaper than the full one, because a large share of both runs is vitest process startup rather than the mutations themselves. Twenty-five seconds off a job that is not on the pipeline's critical path is complexity bought for nothing, and a gate that only runs on the merge queue is a gate whose regression reaches `main` in every pull request that is not queued.
...
- **`fast` (42 rows)** — the invariants whose violation is _silent in production_: a write that repeats or duplicates, an authority check that does not check, a red run reported green, evidence admitted unverified, a dry run that writes. Nobody notices these from a log; they are noticed from a corrupted store or a wrongly-labelled issue weeks later.
- **`full` adds 12 rows** — the invariants whose violation is _loud on the first run_: a roster that stops rotating shows up as one model id in the log; a duty that spends the expensive roster on a pivot shows up on the bill; an empty roster accepted shows up as a run that asked nothing. Still real regressions, still gated, just self-announcing.
...
**The concrete flip condition, so a future maintainer does not have to re-derive it:** when the `mutation` job's own wall clock exceeds roughly a quarter of the `Verify` job's — call it three minutes of harness time, which at the current marginal cost of roughly 1.7 s per row is somewhere near 100 rows — change the job's single run step to:
...
| Disarm                                         | Before                                                                                                                                    | Now                                                                                                                                                                                          |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Empty `MUTATIONS` in `tools/mutation.mjs`      | Printed `KILLED 0 · SURVIVED 0 · STALE 0` and **exited 0**. The top-ranked gate went green because it had been asked to check nothing.    | `TABLE_FLOOR` records the row count. An empty table is refused outright; a table below the floor is refused by name. `tools/mutation.test.mjs` pins the shipped table against its own floor. |
