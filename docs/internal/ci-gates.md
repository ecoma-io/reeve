...
| ---: | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------- |
|    1 | `pnpm test:mutation`                                      | For each named regression, **the suite fails**. This is the only metric here that measures protection rather than execution.                                                    | Only the 60 seams the table names. Silent about everything not in it.                               | 3.5 min harness, ~5 min job | **mandatory (new this round)**      |
|    2 | `pnpm test:tools`                                         | That the two gates above can still **fail**: the mutation harness refuses an emptied or shrunken table, and `vitest.config.ts`'s coverage exclusions and thresholds are pinned. | Anything about the product. It guards the guards.                                                   | <1 s                        | **mandatory (new)**                 |
|    3 | `pnpm test` + coverage floor                              | The suite passes, and no module arrived with no tests at all.                                                                                                                   | That any of it asserts anything. A directory can go from 60% to 95% on tests that only call things. | ~3–4 min                    | mandatory (floor raised this round) |
|    4 | `pnpm test:contract`                                      | The post-1.0 duties contract and the fail-closed exit-code gate hold across every outcome branch.                                                                               | Anything inside a duty.                                                                             | ~1–2 min                    | mandatory (already wired)           |
|    5 | `pnpm eval all`                                           | A fixture run exits 0 only when every outcome was a finding and none failed or skipped.                                                                                         | Behavior no fixture covers.                                                                         | ~1–2 min                    | mandatory (already wired)           |
|    6 | `pnpm typecheck`                                          | esbuild strips types without checking them; this is the only place a type error is caught.                                                                                      | Runtime behavior.                                                                                   | ~30 s                       | mandatory                           |
|    7 | `pnpm lint`                                               | Type-aware; catches a floating promise, an unsafe `any` spread, a `console.log` of a secret.                                                                                    | Behavior.                                                                                           | ~1 min                      | mandatory                           |
|    8 | bundle-staleness check                                    | The committed `dist/` matches `src/` — the single most common way an action repository ships a fix that does nothing.                                                           | That the bundle is correct, only that it is current.                                                | ~1 min                      | mandatory                           |
|    9 | `format:check`, `commitlint`, docs-link and anchor guards | Hygiene the reader sees.                                                                                                                                                        | Behavior.                                                                                           | seconds                     | mandatory                           |
...
- a new `mutation` job (`ci.yml:165-189`) running `pnpm test:mutation`, which now ends `60 KILLED · 0 SURVIVED · 0 STALE`;
...
## The staged plan, and why it is still not staged

The brief asked for a staged strategy so that expensive mutation testing does not run on every trivial change. Measured, on a four-core box:

| Measurement                 | Value                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------ |
| Full mutation table         | 60 mutations, **3 m 27 s** wall clock                                                            |
| `--stage fast` subset       | 44 mutations, **1 m 07 s**                                                                       |
| The 4 entry-point rows      | roughly **35 s each** — a bundle rebuild plus a spawned child                                    |
| `pnpm test` with coverage   | 4868 tests over 187 files, **~250 s**                                                            |
| The `Verify` job as a whole | lint, typecheck, test, contract, eval, two builds — **minutes**, and the pipeline's longest pole |

The full table nearly quadrupled this round, and all of the growth is in four rows. Entry-point mutations are genuinely expensive: each one rebuilds a duty bundle with esbuild and then spawns a child process to run it, which is why they are `"full"` and not `"fast"`.

**The recommendation is unchanged: the full table on every event —** `pull_request`, `merge_group`, `push: main` and `workflow_dispatch` — which is what is wired. Two reasons, and the second is the one that would survive a disagreement about the first:

1. **The mutation job runs in parallel with `Verify` and is not the longest pole.** At 3 m 27 s against a `Verify` job of several minutes, it still finishes first, so the wall clock the pipeline adds for it is zero. Staging would save 2 m 20 s off a job nobody is waiting for.
2. **The four rows that staging would defer are the four that caught the blocking finding.** They are the publish and capability gates in `dependa`, `harmonise`, `lifecycle` and `triage` — the last check between a run and a repository mutation. Moving them behind the merge queue means a pull request that opens one of those gates goes green on the pull request. That is a real reduction in what a review proves, bought with time nobody was spending.

### Correcting the flip condition

An earlier version of this document set the flip condition at "roughly a quarter of the `Verify` job's wall clock — call it three minutes of harness time". The table now measures 3 m 27 s, so on the letter of that rule it should be flipped. It should not be, and the rule was wrong rather than the measurement.

Three minutes was a proxy, and the thing it was proxying for is whether the mutation job becomes the pipeline's critical path. It has not: a job running in parallel costs nothing until it becomes the longest one. Stating the criterion as a fixed number invited exactly this — a rule that fires before the condition it stands for is true.

**The corrected condition: flip when the `mutation` job's wall clock approaches the `Verify` job's, so that it starts determining when the pipeline finishes.** That is a comparison between two numbers CI already reports, not a constant that has to be re-derived. When it happens, change the job's single run step to:
...
That keeps `merge_group` and `push: main` on the full table, which is what actually gates `main`. Even then, consider promoting the four entry-point rows to `fast` rather than deferring them, and deferring something cheaper instead: the tier is a judgement about blast radius, and a row's cost is not the same thing as its worth.

### What is in each tier

- **`fast` (44 rows)** — the invariants whose violation is _silent in production_: a write that repeats or duplicates, an authority check that does not check, a red run reported green, evidence admitted unverified, a dry run that writes. Nobody notices these from a log; they are noticed from a corrupted store or a wrongly-labelled issue weeks later.
- **`full` (16 rows)** — two kinds. The first is the invariants whose violation is _loud on the first run_: a roster that stops rotating shows up as one model id in the log; a duty that spends the expensive roster on a pivot shows up on the bill. The second, new this round, is the four **entry-point** rows, which are `"full"` purely because they cost a rebuild — not because they matter less. They matter most.
...
## What guards the guards

An adversarial review that wrote none of this round asked the question this section exists to answer: **can each gate be made to report success by disarming it, rather than by doing the work?** For three of them the answer was yes, and all three are now closed. None of the fixes is a guarantee, and the difference matters enough to state plainly.

| Disarm                                         | Before                                                                                                                                    | Now                                                                                                                                                                                               |
| ---------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Empty `MUTATIONS` in `tools/mutation.mjs`      | Printed `KILLED 0 · SURVIVED 0 · STALE 0` and **exited 0**. The top-ranked gate went green because it had been asked to check nothing.    | `TABLE_FLOOR` records the row count (60). An empty table is refused outright; a table below the floor is refused by name. `tools/mutation.test.mjs` pins the shipped table against its own floor. |
| Delete a duty from `eval/runner.ts`'s `DUTIES` | `pnpm eval all` exited 0 having silently skipped it. For `dependa` and `triage` an eval fixture is the only end-to-end evidence there is. | `eval/contract/duties.test.ts` reads the runner's list as text and asserts it against `src/refusal.ts`'s `DUTIES`, which is itself pinned to the directories carrying an `action.yml`.            |
| Add a glob to `coverage.exclude`               | Raised every coverage percentage and turned nothing red. The rule against it existed only as a comment.                                   | `tools/coverage-config.test.mjs` pins the exclusion list, the `include` glob, and all four thresholds at or above 90.                                                                             |

**What these three cannot do.** None of them stops somebody editing the check and the thing it checks in the same commit — nothing in a repository can, and a document that implied otherwise would be doing the same thing this section is about. `TABLE_FLOOR` can be lowered. `RECORDED_EXCLUSIONS` can be extended. The coverage thresholds can be edited, and so can the test that pins them.

What they change is the _kind_ of act required. Before, shrinking a gate was a one-line edit that produced a green build and looked like an improvement — the number went up. Now it takes a second, explicit edit, in a file whose entire subject is that the first edit is suspicious, carrying a comment that says what the correct fix is instead. That converts a silent disarm into a deliberate one that has to survive review. It is the same standing the coverage thresholds have always had, and it is worth exactly as much as a reviewer reading the diff.

**So: these are deliberateness pins, not guarantees.** The honest one-line summary is that this repository has no gate that a determined committer cannot switch off, and the design goal is instead that no gate can be switched off _by accident_ or _while appearing to improve something_.

### The one still-unenforced rule

`vitest.config.ts`'s exclusion list is pinned, but the _justification_ for each entry is not. `tools/coverage-config.test.mjs` asserts that the list is exactly the three recorded globs; it does not and cannot assert that each excluded entry point is genuinely exercised elsewhere. That gap is not hypothetical — two of the nine files matched by `src/duties/*/main.ts` are executed by no test at all, which [the coverage baseline](coverage-baseline.md#the-exclusions-restated) now names.

What would enforce it: a test asserting that every duty directory carries a `main.integration.test.ts` which actually spawns its bundle, in the shape `src/refusal.test.ts` already uses to bind `DUTIES` to the directories holding an `action.yml`. That is a real and cheap check, and it is deliberately **not** written here — it would go red today, and landing a gate that is red on arrival at the close of a hardening round buries the two findings it exists to surface. It belongs with the work that closes them. Owner: TL2.
...
- **`tools/` is outside the coverage `include` glob**, so `tools/mutation.mjs` contributes to no coverage number. It is no longer untested: `pnpm test:tools` covers the refusal paths — empty table, table below floor, zero-match `from`, ambiguous `from` — which are the branches that by construction never execute during a passing run and can therefore rot indefinitely unnoticed. What is still uncovered there is the I/O half: the crash-recovery pass and the signal handlers were verified by hand once and nothing re-verifies them.
