...
| `review`     | Medium      | Pull request review. Deterministic pre-checks plus one model pass, with model findings verified against deterministic evidence before they are reported, as a single owned comment. Report-only at level 0 — needs a `duties:` grant in the warrant to act. |
