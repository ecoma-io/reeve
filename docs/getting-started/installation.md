...
| `review`     | Medium      | Pull request review. Deterministic pre-checks plus risk-tiered model passes (one on low-risk diffs, more on high-risk ones), with model findings verified against deterministic evidence before they are reported, synthesized into a single owned summary comment plus an owned inline thread per diff-proven finding. Report-only at level 0 — needs a `duties:` grant in the warrant to act. |
