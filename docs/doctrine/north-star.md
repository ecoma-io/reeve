...
| `review`      | ships  | Reviews a pull request — deterministic pre-checks plus risk-tiered model passes (one on low-risk diffs, more on high-risk ones), with model findings verified against deterministic evidence before they are reported, kept as one owned summary comment plus an owned inline thread per diff-proven finding across synchronize events. |
...
the same dogfooding the per-thread jobs in the same file already do.
